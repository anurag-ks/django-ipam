...
class BaseTestModel(object):
    ...
    def test_invalid_ipaddress_subnet(self):
        failed = True
        self._create_subnet(dict(subnet='192.168.2.0/24'))
        try:
            self._create_ipaddress(dict(ip_address='10.0.0.2',
    ...
    def test_valid_ipaddress_subnet(self):
        ...
        self._create_subnet(dict(subnet='192.168.2.0/24'))
        try:
            self._create_ipaddress(dict(ip_address='192.168.2.1',
                                        subnet=self.subnet_model.objects.first()))
    ...
    def test_valid_cidr_field(self):
        failed = False
        try:
            self._create_subnet(dict(subnet='22.0.0.0/24'))
        except ValidationError:
            failed = True
        if failed:
            self.fail('ValidationError raised')
    ...
    def test_invalid_cidr_field(self):
        ...
        try:
            self._create_subnet(dict(subnet='192.192.192.192.192'))
        except ValidationError as e:
    ...
    def test_overlapping_subnet(self):
        ...
        self._create_subnet(dict(subnet='192.168.2.0/24'))
        try:
            self._create_subnet(dict(subnet='192.168.2.0/25'))
        except ValidationError as e:
    ...
    def test_save_none_subnet_fails(self):
        failed = True
        try:
            self._create_subnet(dict(subnet=None))
        except ValidationError:
            failed = False
        if failed:
            self.fail('ValidationError not raised')
    ...
    def test_save_blank_subnet_fails(self):
        failed = True
        try:
            self._create_subnet(dict(subnet=""))
        except ValidationError:
            failed = False
        if failed:
            self.fail('ValidationError not raised')
    ...
    def test_retrieves_ipv4_ipnetwork_type(self):
        instance = self._create_subnet(dict(subnet='10.1.2.0/24'))
    ...
    def test_retrieves_ipv6_ipnetwork_type(self):
        instance = self._create_subnet(dict(subnet='2001:db8::0/32'))
        instance = self.subnet_model.objects.get(pk=instance.pk)
